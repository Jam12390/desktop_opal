...
def initLog():
    ...
    onedrivePath = f"{pathlib.Path.home()}\\OneDrive\\Documents\\DesktopOpal\\ErrorLog.txt" #I AM GOING TO KILL ONEDRIVE. FOR SOME UNHOLY REASON FLUTTER CREATES ERRORLOG.TXT IN ONEDRIVE >:(
    print(path)
    try:
        errorLog = open(path, "a")
    except:
        errorLog = open(onedrivePath, "a")
    print(errorLog)
...
def terminateBlockedApps():
    ...
    try:
        ...
        errorLog.write(formatErrorLog(logtype="WARNING", log=f"Apps failed to terminate due to {e}.")) # type: ignore
...
@api.post("/kill")
def killSelf(params: CrashMsg):
    for process in psutil.process_iter(["name"]):
        if process.info["name"] == "desktop_opal.exe":
            process.terminate()
            ctypes.windll.user32.MessageBoxW(0, f"Critical error occurred!\n {CrashMsg.exception}.", 1)
            server.should_exit = True
...
@api.post("/notify")
def showNotif(notifData: NotifRequest):
    toast(
        notifData.notifTitle,
        notifData.notifSubtitle,
        audio=notifData.audio,
        duration="short",
        button="Dismiss"
    )
...
def getAutoExecutables():
    ...
    if len(combinedExecutables) == 0:
        errorLog.write(formatErrorLog(logtype="WARNING", log="No executables located at common desktop paths.")) # type: ignore
...
os.system("start appdata\\appFrontend.exe") #TODO: in build versions, make sure this is run